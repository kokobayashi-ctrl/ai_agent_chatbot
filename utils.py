...
def create_rag_chain_from_uploaded_files(uploaded_files):
    """
    アップロードされたファイルからRAGチェーンを作成
    ...
    for uploaded_file in uploaded_files:
        ...
        try:
            ...
            if file_extension == ".pdf":
                ...
                loader = ct.SUPPORTED_EXTENSIONS[".txt"](tmp_file_path)
            else:
            ...
            logger.error(f"ファイル読み込みエラー ({uploaded_file.name}): {e}")
    ...
    question_answer_template = ct.SYSTEM_PROMPT_INQUIRY_UPLOADED
...
def execute_agent_or_chain(chat_message):
    ...
    # アップロードされたファイルがある場合は、そちらを優先的に使用
    if "uploaded_rag_chain" in st.session_state and st.session_state.uploaded_rag_chain:
        result = st.session_state.uploaded_rag_chain.invoke({
            "input": chat_message, 
    ...
    # AIエージェント機能を利用する場合
    elif st.session_state.agent_mode == ct.AI_AGENT_MODE_ON:
